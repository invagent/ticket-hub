...
import { DefaultPoolTab } from "./DefaultPoolTab";
import { ScopeHistoryTab } from "./ScopeHistoryTab";

/**
 * /admin/scopes — admin-only routing scope management.
 *
 *   Modules tab      product_line × module → user_id（路由 step 1）
 *   Features tab     feature → user_id（路由 step 2 兜底）
 *   DefaultPool tab  全局兜底处理人（路由 step 3 最终保底）
 *   History tab      add/remove 审计
 *
 * Page is rendered for any logged-in user; backend enforces role=admin/supervisor.
 * Non-admins will see 403 errors inline when they try to add/delete.
 */
type Tab = "modules" | "features" | "default-pool" | "history";
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-500">
        路由分工：module 优先 → feature 兜底 → 全局兜底（决策
        D20）。变更全审计。
      </p>
      <div className="border-b border-gray-200 dark:border-gray-800">
        <nav className="flex gap-1">
          ...
          <TabButton
            active={tab === "default-pool"}
            onClick={() => setTab("default-pool")}
          >
            全局兜底
          </TabButton>
          <TabButton
      ...
      {tab === "default-pool" && <DefaultPoolTab />}
