/**
 * 反思诊断共享列组件（从 ReflectWorkbenchPage.tsx 抽出，供 `/reflect` 全屏工作台
 * 与工单详情页的反思抽屉共用同一份实现，不重复造轮子）。
 *
 * `DiagnosisColumn`/`RemedyColumn` 本身是自包含、宽度弹性的组件：只吃
 * ticketId/ctx（+RemedyColumn 还吃 aiCsEnabled/qc），内部自带查询/mutation。
 * `stacked` prop 为 true 时去掉 flex-[N]/border-r（供纵向堆叠容器，如抽屉，使用），
 * 默认 false 保持原 `/reflect` 页面并排布局不变。
 */
...
export type ConvTurn = { role?: string; text?: string; ts?: string };
...
export type FileEdit = { filename: string; filepath: string; content: string };
...
export const CAUSE_HINTS: Record<string, string> = {
  skill: "修订 skill 提示词可解 → 右侧开药，改完创建 draft 后 replay 验证。",
  knowledge:
    "下方「知识库核对」查到的条目内容有误/过期 → 去飞书更正该条目，点「重新检索」确认，再 replay（当前 skill + 最新知识）验证。",
  retrieval:
    "下方「知识库核对」无命中 = 知识缺失 → 去飞书补充该主题条目，点「重新检索」确认命中后再 replay 验证。",
};
...
export function citeKey(c: Cited): string {
  return c.id || c.url || c.title || "";
}

/* ═══════════ 第 1 段：看症状（诊断区） ═══════════ */

export function DiagnosisColumn({
  ticketId,
  ctx,
  stacked = false,
}: {
  ...
  return (
    <div
      className={`min-w-0 flex flex-col bg-white ${stacked ? "" : "flex-[5] border-r border-[#e8e3d9]"}`}
    >
      <div
        className="flex-none px-4.5 py-3 border-b border-[#efeae0] flex items-center gap-2"
        style={{ padding: "12px 18px 10px" }}
      >
        <span className="w-[18px] h-[18px] flex-none rounded-full bg-[#2b2a26] text-white text-[10.5px] font-bold flex items-center justify-center">
          1
        </span>
      ...
      <div
        className={`min-h-0 flex flex-col gap-3.5 ${stacked ? "" : "flex-1 overflow-auto"}`}
        style={{ padding: "14px 18px 20px" }}
      >
        {/* 黄金三元组 */}
        <div className="flex flex-col gap-2">
          <div className="border border-[#e8e3d9] rounded-lg px-3 py-2.5 bg-[#fbf9f5]">
            <div className="text-[10.5px] font-bold text-[#8b8577] tracking-wide mb-0.5">
              客户原问题
            </div>
          ...
          <div className="border border-[#eed7d2] rounded-lg px-3 py-2.5 bg-[#fbf1ef]">
            <div className="text-[10.5px] font-bold text-[#b04a4a] tracking-wide mb-0.5">
              AI 原答复 · 生产版
            </div>
            <div className="text-[12.5px] text-[#5c4340] whitespace-pre-wrap">
              {ctx.ai_answer || "（无）"}
            </div>
          </div>
          {ctx.dissatisfaction && (
            <div className="border border-[#eddfba] rounded-lg px-3 py-2.5 bg-[#faf3e3]">
              <div className="text-[10.5px] font-bold text-[#9a6c1c] tracking-wide mb-0.5">
                {ctx.is_ai_cs_escalation ? "客户不满反馈（转人工原因）" : "内部复核意见"}
              </div>
              <div className="text-[12.5px] font-semibold text-[#6d5320]">
                「{ctx.dissatisfaction}」
              </div>
            </div>
          )}
        ...
        {conversation.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-bold text-[#57524a]">
                完整会话（{conversation.length} 条）
              </span>
        ...
        {/* 引用的知识 */}
        <div>
          <div className="text-xs font-bold text-[#57524a] mb-1">
            原答复引用的知识（{cites.length}）
          </div>
              ...
              {cites.map((c, i) => {
                ...
                return (
                  <div key={i} className="border border-[#e8e3d9] rounded-lg px-3 py-2 bg-white">
                    <div className="flex items-center gap-2">
                      ...
                      {c.url ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-[12.5px] font-semibold hover:underline truncate"
                        >
                          {c.title || c.id || `知识${i + 1}`}
                        </a>
                      ) : (
                    ...
                    {low && (
                      <div className="text-[11px] text-[#b04a4a] mt-1">
                        低相似度 — 强信号：检索可能没命中对的知识
                      </div>
                    )}
                    {exp && c.snippet && (
        ...
        <div className="border border-[#ddd6c8] rounded-[10px] bg-[#fbf9f5] px-3.5 py-2.5">
          <div className="flex items-center gap-1.5 mb-1.5">
            <span className="w-[15px] h-[15px] flex-none rounded bg-[#57524a] text-white text-[10px] font-bold flex items-center justify-center">
              ✓
            </span>
            <span className="text-[11.5px] font-bold text-[#57524a]">
              人工核对的正确答案（诊断基准）
            </span>
            {answerDraft === null && (
              <button
                onClick={() => setAnswerDraft(correctAnswer)}
                className="ml-auto text-[11px] text-[#177e83]"
              >
                {correctAnswer ? "编辑" : "填写"}
              </button>
            )}
        ...
        <div className="border-[1.5px] border-[#9fc9c7] rounded-[10px] bg-[#f2f8f7] px-3.5 py-3">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[12.5px] font-bold text-[#14666a]">
              AI 反思推断 · 对照正解逐步排查
            </span>
            <button
                ...
                {(reflection.steps ?? []).map((rs, i, arr) => (
                  <div key={i} className="flex gap-2">
                    <div className="flex-none flex flex-col items-center w-[18px]">
                      ...
                      {i < arr.length - 1 && (
                        <span className="w-[1.5px] flex-1 min-h-[10px] bg-[#cfe4e2]" />
                      )}
                    </div>
                ...
                <div className="flex items-baseline gap-1.5 flex-wrap">
                  ...
                  {typeof reflection.confidence === "number" && (
                    <span className="text-[10.5px] text-[#8b8577]">
                      置信 {(reflection.confidence * 100).toFixed(0)}%
                    </span>
                  )}
                </div>
        ...
        {checklist.length > 0 && (
          <div
            className={`border-[1.5px] rounded-[10px] px-3.5 py-3 ${
              diagnosis?.resolved ? "border-[#9ecdb2] bg-[#eef7f1]" : "border-[#e3d9be] bg-[#fbf7ec]"
            }`}
          >
            <div className="flex items-center gap-2 mb-2">
              <span
...
export function KbCheckPanel({ question, causes }: { question: string; causes: string[] }) {
  ...
  const search = useQuery({
    ...
    queryFn: () => api.get("/api/supervisor/kb/search", { q: question, limit: 5, force: force > 0 }),
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[10.5px] text-[#8b8577]">飞书 · {status.data?.doc_count ?? "…"} 篇</span>
        <button
      ...
      {status.data?.error ? (
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {hits.map((h) => (
            <div key={h.node_token} className="bg-white border border-[#e8e3d9] rounded-[7px] px-2.5 py-2">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold flex-1 min-w-0 truncate">{h.title}</span>
                <span className="text-[9.5px] text-[#8b8577] font-mono flex-none">相关 {h.score}</span>
                {h.url && (
                  <a
                    href={h.url}
                    target="_blank"
...
export function CiteTag({ type }: { type?: string }) {
...
export function RemedyColumn({
  ...
  if (!aiCsEnabled) {
    return (
      <div className={`min-w-0 flex flex-col ${stacked ? "" : "flex-[6]"}`}>
  ...
  return (
    <div className={`min-w-0 flex flex-col ${stacked ? "" : "flex-[6]"}`}>
      {/* 第 2 段：开药 */}
      <div
        className={`min-h-0 flex flex-col bg-white border-b border-[#e8e3d9] ${stacked ? "" : "flex-[23]"}`}
      >
        <div
          className="flex-none border-b border-[#efeae0] flex items-center gap-2"
          style={{ padding: "12px 18px 10px" }}
        >
          <span className="w-[18px] h-[18px] flex-none rounded-full bg-[#2b2a26] text-white text-[10.5px] font-bold flex items-center justify-center">
            2
          ...
          <span className="ml-auto text-[10.5px] text-[#8b8577]">
            生产版：{detail.data?.published_version ?? "…"}
          </span>
        ...
        <div
          className={`min-h-0 flex flex-col gap-2.5 ${stacked ? "" : "flex-1 overflow-auto"}`}
        ...
        >
          ...
          <textarea
            ...
            onChange={(e) =>
              setEdits((prev) =>
                prev.map((x, j) => (j === activeIdx ? { ...x, content: e.target.value } : x)),
              )
            }
          ...
          <div className="flex items-center gap-2 text-[11px] min-h-4">
            {draftVersion ? (
              <span className="text-[#177e83] font-semibold">
                修改已计入 {draftVersion} — 未发布，不影响生产
              </span>
            ) : dirty ? (
      ...
      {/* 第 3 段：试药与处方 */}
      <div className={`min-h-0 flex flex-col bg-[#fbf9f5] ${stacked ? "" : "flex-[27]"}`}>
        <div
          className="flex-none border-b border-[#efeae0] flex items-center gap-2"
          style={{ padding: "12px 18px 10px" }}
        >
        ...
        >
          {published && (
            <div className="border border-[#bcd9c4] rounded-lg bg-[#edf5ee] px-3.5 py-2.5 flex items-center gap-2.5">
              <span className="w-[18px] h-[18px] flex-none rounded-full bg-[#2f7d4f] text-white text-[11px] font-bold flex items-center justify-center">
                ✓
              </span>
              <div>
            ...
            <>
              <div className="grid grid-cols-2 gap-2.5 items-start">
                <div className="border border-[#eed7d2] rounded-[9px] bg-white overflow-hidden">
                  ...
                  <div className="px-3 py-2.5 text-xs text-[#57524a] whitespace-pre-wrap">
                    {ctx.ai_answer || "（无）"}
                  </div>
