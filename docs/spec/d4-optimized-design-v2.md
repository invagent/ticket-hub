...
### Phase 2 — KSM 回写闭环（需求 6 另一半）✅ 代码完成（2026-06-26）
| # | 项 | 状态 |
|---|---|---|
| 2.1 | sync_outbox KSM sender：reply→lock+handle(is_deal)、status in_progress→lock、status released→handle 关单；lock→重拉 node→handle 时序；NoticeStore 刷新 + 失败回落；灰度开关 `ksm_writeback_enabled`(关)+`ksm_writeback_dry_run`(开)；失败 attempts/last_error 转人工；Celery beat 2min + 主管 `POST /api/supervisor/drain-ksm-writeback` | ✅ 8a13875，20 单测 |
| 2.2 | KSM 补料 supply：迁移 0016 扩 outbox kind 加 'supply'；`request_supply` cascade + 主管 `POST /api/hub-issues/{id}/request-supply`；sender supply 分支 lock→refresh→supply_order | ✅ fdba64a，9 单测 |

> **主项目 `adapters/ksm/KSMClient` 早已有全套写方法**（lock/handle/supply/return/get_order_detail），无需移植——Phase 2 只补了**消费 sync_outbox 的 sender**。
> **上线步骤（用户执行）**：① 服务器 `git pull` + `alembic upgrade head`（应用 0016）+ 重启 systemd（backend/celery-worker/celery-beat）；② `.env` 配 `KSM_HANDLER_NAME` / `KSM_HANDLER_NUMBER` + 生产 `KSM_BASE_URL=ierp.kingdee.com`；③ 先 `KSM_WRITEBACK_ENABLED=true` 保持 `KSM_WRITEBACK_DRY_RUN=true` 观察组装 payload；④ 确认无误后翻 `KSM_WRITEBACK_DRY_RUN=false` 真打 KSM UAT/prod。
> **智齿回写**：本期未做（outbox 已留 zhichi 行，sender 只 drain ksm）；同 sample `save_ticket_reply` 套路，后续单独补。
> **飞书 KB 写**（原 2.2 旧条目）：属 Phase 1 知识反哺发布环节，随 Phase 1 一起做（阻塞于飞书空间编辑权 131006）。
