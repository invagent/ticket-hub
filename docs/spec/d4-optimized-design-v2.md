...
### Phase 0 — 无外部依赖，立即可做（约 1.5 周）
| # | 项 | 依赖 | 工作量 |
|---|---|---|---|
| 0.1 | PII 脱敏移植（desensitize）| 无 | 0.5d |
| 0.2 | `skill_prompts` DB 化提示词 + 编辑/版本/回滚/预览 API + 前端 Skill 配置页（需求 2/6 热加载）| 无 | 4d |
| 0.3 | hub-dedup 建 Linear 前语义去重（需求 4）| 无 | 2d |
| 0.4 | 90 天语义重复挂载 + 主管纠偏（需求 5）| 无 | 1.5d |
| 0.5 | SLA 工作日/节假日（需求 6 一半）| 无 | 1.5d |
