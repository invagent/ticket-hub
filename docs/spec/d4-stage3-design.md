...
1. **不满意信号来源**：AI 客服有事件回调吗？没有的话接受轮询延迟（分钟级）吗？
2. **PII 边界**：接受国内多模态（qwen-vl/GLM-4V），encryptor 出第③段？
3. **KSM 附件**：下载接口/凭证形态？
4. **知识反哺**：AI 客服有知识库导入 API 吗？要不要做这个飞轮？
5. **Operation 复杂问题转人工**：escalation 判回 Operation 时，走 hub 人工
   回复（reply_sync）还是转回 AI 客服系统的人工坐席？（影响工单归属）
```
