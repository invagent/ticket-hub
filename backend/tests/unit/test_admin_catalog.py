...
def test_same_module_name_under_different_product_line_ok(
...
def test_list_modules_filter_inactive(app_client, db_session: Session) -> None:
    ...
    db_session.add_all(
        [
            ...
            Module(
                product_line_code="cloud-fapiao", name="inactive-mod", is_active=False
            ),
        ]
    )
    db_session.commit()
...
def test_create_feature_then_list(app_client, world: Session) -> None:
    r = app_client.post(
        "/api/admin/features", headers=_admin_bearer(), json={"name": "数据导入"}
    )
...
def test_feature_duplicate_409(app_client, world: Session) -> None:
    app_client.post(
        "/api/admin/features", headers=_admin_bearer(), json={"name": "x"}
    )
    r = app_client.post(
        "/api/admin/features", headers=_admin_bearer(), json={"name": "x"}
    )
...
def test_delete_feature(app_client, world: Session) -> None:
    r = app_client.post(
        "/api/admin/features", headers=_admin_bearer(), json={"name": "y"}
    )
    fid = r.json()["id"]
